...
const formSchema = z.object({
  name: z.string().min(1, "Server name is required").max(20, "Name must be less than 20 characters"),
...
const EditServerModal = () => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      if (!server?.id) throw new Error("Server ID missing");
      ...
      console.error("[SERVER_EDIT_ERROR]", error.message);
      alert("Failed to edit server: " + error.message);
    }
  };
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[425px] sm:max-h-[85vh] bg-white text-black overflow-hidden text-center">
        <DialogHeader>
          <DialogTitle className="font-bold">Customize your server</DialogTitle>
          <DialogDescription>
            Yea your Server is Booooooring
          </DialogDescription>
        </DialogHeader>
